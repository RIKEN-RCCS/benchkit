...
case "$system" in
  ...
  RC_DGXSP)
    ...
    MATHLIBS_ROOT="$(dirname "$(dirname "$(dirname "$(command -v nvcc)")")")/math_libs"
    MATHLIBS_CANDIDATES=("${MATHLIBS_ROOT}"/*/lib64)
    if [ "${#MATHLIBS_CANDIDATES[@]}" -ne 1 ] || [ ! -d "${MATHLIBS_CANDIDATES[0]}" ]; then
      echo "Expected one NVHPC math_libs/*/lib64 directory under ${MATHLIBS_ROOT}" >&2
      exit 1
    fi
    MATHLIBS="${MATHLIBS_CANDIDATES[0]}"
    (
      cd "${PETSC_DIR}"
      ./configure \
        --with-cc=mpicc --with-cxx=mpicxx --with-fc=0 \
