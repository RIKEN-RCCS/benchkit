...
# Append flags without losing values that a site module or CI variable already
# set before this build script runs.
append_env_flags() {
    local var_name="$1"
    local new_flags="$2"
...
# Export CUDA include/library flags in the forms GENESIS configure currently
# checks. The sbsa-linux paths are needed by GH200-style NVHPC installations.
...
# GENESIS still includes the legacy NVTX header. Newer CUDA/NVHPC stacks install
# the compatibility header below nvtx3, so patch the checked-out source locally.
...
# The upstream configure.ac recognizes pgfortran but not nvfortran. Treating
# nvfortran as the same compiler family keeps the rest of GENESIS' PGI/NVHPC
# configuration path active without carrying a fork of the source tree.
...
# GENESIS releases vary between shipping bootstrap and relying on autoreconf.
bootstrap_genesis() {
    if [ -x ./bootstrap ]; then
        bash ./bootstrap
    else
...
configure_genesis_gh200_gpu() {
    ...
    # GENESIS configure probes compiler versions through this variable.
    version="--version"
    FC="${!fc_var:-mpif90}"
    CC="${!cc_var:-mpicc}"
    CXX="${!cxx_var:-mpicxx}"
    ...
    # the corresponding site paths are available.
