...
def test_profile_usage_overview_links_profile_triggers_results_and_node_hours(tmp_path):
    ...
    store.create_trigger_run(
        trigger_id="demoapp-demosystem-time",
        trigger_type="scheduled",
        status="submitted",
        dry_run=False,
    ...
    row = overview["rows"][0]
    assert row["profile_id"] == "demoapp-demosystem"
    assert row["allocation_project_id"] == "project00010"
    ...
    assert row["latest_result"]["trigger_headline"] == "Scheduled / demoapp-demosystem-time"
    ...
    assert row["latest_result"]["environment_snapshot"]["allocation_project_id"] == "project00010"
...
def test_profile_usage_overview_does_not_scope_match_triggered_results_to_other_profiles(tmp_path):
    ...
    store = ExecutionProfileStore(str(db_path))
    store.upsert_profile(_profile(id="demoapp-demosystem"), actor="admin@test.com")
    ...
    store.upsert_trigger_definition(_trigger(profile_id="demoapp-demosystem"), actor="admin@test.com")
        ...
        json.dumps(
            {
                ...
                "execution_trigger": {
                    "id": "demoapp-demosystem",
                    "type": "manual_button",
                    "reason": "manual_button:demoapp-demosystem",
                },
            }
        ),
        encoding="utf-8",
    ...
    assert rows["demoapp-demosystem"]["result_count"] == 3
    assert rows["demoapp-demosystem"]["node_hours"] == 1.75
    assert rows["demoapp-demosystem"]["attribution_counts"] == {
        "trigger_id_match": 1,
        "manual_profile_match": 1,
        "legacy_scope_fallback": 1,
    }
    assert rows["demoapp-demosystem"]["latest_result"]["attribution"]["reason"] == "manual_profile_match"
    assert rows["demoapp-test"]["result_count"] == 1
    assert rows["demoapp-test"]["node_hours"] == 0.5
    assert rows["demoapp-test"]["attribution_counts"] == {
        "trigger_id_match": 0,
        "manual_profile_match": 0,
        "legacy_scope_fallback": 1,
    }
    assert rows["demoapp-test"]["latest_result"]["timestamp"] == "2026-08-09 17:00:00"
    assert rows["demoapp-test"]["latest_result"]["attribution"]["reason"] == "legacy_scope_fallback"
