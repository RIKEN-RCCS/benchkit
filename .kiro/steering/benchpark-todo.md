...
- 結果送信スクリプト（`send_benchpark_results.sh`）の実装
...
### 3. 結果変換機能の実装
- [x] `convert_benchpark_results.py`の動作確認
- [x] BenchKit形式への変換テスト
- [x] 複数実験対応（3つ以上の実験を個別JSONファイルとして出力）
- [x] メトリクス抽出ロジックの修正（FOM値が正しく抽出される）
- [x] 結果送信スクリプト（`send_benchpark_results.sh`）の実装
- [ ] 結果表示システムでの表示確認（実際のサーバへのPOSTテスト）
...
- [ ] **ベクトル的なFOM構造への拡張**
  ...
  - 例: `{"message_size": [1, 2, 4, 8], "bandwidth": [100, 200, 300, 400]}`
- [ ] BenchKit結果サーバの表示形式改善
  - グラフ表示機能の追加
  - メッセージサイズ vs バンド幅のプロット
  - 複数メトリクスの同時表示
- [ ] 複数メトリクスの統合表示方法の検討
- [ ] 結果変換スクリプトの大幅な修正（将来的な拡張）
