...
# Keep only application integration evidence here; recorder semantics are common.
mapfile -t timing_files < <(find "${RESULTS_DIR}" -maxdepth 1 -name 'workflow_timing_*.json' | sort)
test "${#timing_files[@]}" -eq 1
jq -e '
  .kind == "workflow_stage_timing" and .producer == "benchkit" and
  .exp == "timing-test" and
  ...
  ([.stages[] | select(.stage == "collect" and .tool == "ncu")] | length) == 5 and
  any(.stages[]; .stage == "export" and .tool == "ncu") and
