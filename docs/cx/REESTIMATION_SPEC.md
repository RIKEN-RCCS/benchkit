...
- 推定モデル更新後の再評価
- 推定条件変更後の再評価
- `weakscaling` を最小経路とする推定と詳細推定の比較
- 将来システム仮定を変えた比較
...
- comparison between `weakscaling`-based minimum estimation and detailed estimation
...
すなわち、同じ UUID に対して、`weakscaling` を最小経路とする推定、詳細推定、外部モデル推定などを並存させられることが望ましい。

Re-estimation assumes replaceable estimation methods.
For the same UUID, it should preferably support coexistence of `weakscaling`-based minimum estimation, detailed estimation, and external-model-based estimation.
...
- artifact 不要な `weakscaling` 再推定は成立しうる
...
- `weakscaling` or artifact-free re-estimation may succeed
...
- 不適用として終了する条件
- `weakscaling` などの最小経路へフォールバックしてよい条件
- 再計測や追加準備を要求する条件
