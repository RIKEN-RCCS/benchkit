...
- `weakscaling` を最小経路としつつ、より詳細な推定が共存できること
...
- a weakscaling-based minimum estimation path and more detailed estimation must coexist
...
- 最小構成の区間分割推定:
  実測 FOM を section / overlap の時間分解と組み合わせ、`weakscaling` を最小の参照経路として行う推定
- 詳細推定:
...
- minimum section-wise estimation:
  estimation using measured section / overlap timing or equivalent decomposition, with `weakscaling` as the minimum reference path
- detailed estimation:
  estimation using application-defined interval timings, tool-derived interval timings, traces, detailed counters, or other richer inputs
...
- what level of section / overlap breakdown the collection provides
...
section 分解、overlap、通信区間時間、またはそれに準ずる情報が無い入力に対しては、通信成分補正や同期オーバーヘッド補正を前提としてはならない。
...
Estimation inputs without section breakdown, overlap, communication interval timing, or equivalent information must not assume communication-cost or synchronization-overhead adjustment.
...
- 要求された推定パッケージ
- 実際に適用されたフォールバック先パッケージ
- 要求されたパッケージを適用できなかった具体的理由

When applicability evaluation returns fallback, it is desirable that the result make all of the following identifiable:
...
- 実測 FOM
- section / overlap の時間分解、またはそれに相当する実行時間の分解
- 基準 benchmark result
- 各 system の target node count
- 各 component に適用する package-defined scaling rules

用途:

- 同一 system line 内での scaling
- section breakdown を使った current-side 推定
- より豊かな取得入力はまだ無いが、section / overlap timing は出せる app
...
- measured FOM
- section / overlap timing breakdown, or equivalent decomposition of the measured execution time
- baseline result
- target node count for each system
- package-defined scaling rules for each component

Typical use:

- same-system-line scaling
- current-side estimation from measured section breakdown
- applications that can already expose section / overlap timing but do not yet provide richer acquisition inputs

最小経路では、FOM を単独で使うのではなく、section / overlap の時間分解と組み合わせて各 component に package ごとの scaling rule を適用する。
追加補正を入れる場合は、`weakscaling` の基本形ではなく、補正付きの変種またはより詳細な推定方式として扱うべきである。

In the minimum path, FOM is not used by itself; it is interpreted together with a section / overlap breakdown, and package-specific scaling rules are applied to each component.
If additional correction terms are introduced, they should be treated not as the default `weakscaling` path, but as a corrected variant or a more detailed estimation method.
