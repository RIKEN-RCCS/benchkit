...
| package metadata | package 名、版、required inputs、fallback policy を持つこと | `weakscaling` / 詳細ダミーとも最小 metadata を持つ。現状 `method_class=lightweight` は互換のため残っているが、意味としては `weakscaling` を最小経路とする class である | richer metadata を discovery や UI に活かす実装、`method_class` 命名の後追い整理がまだ無い | 中 |
