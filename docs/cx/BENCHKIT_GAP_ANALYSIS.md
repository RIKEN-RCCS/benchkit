...
| section ごとの package 指定 | 区間ごとに推定 package を割り当てられること | `bk_emit_section` / `bk_emit_overlap` から Result JSON に `estimation_package` を載せられ、`instrumented_app_sections_dummy` でも dispatch に利用している | 他 app への横展開と package discovery の整理が未完 | 最優先 |
...
1. `estimate.sh` のテンプレートまたは共通補助を整える
2. `qws` 以外に 1 から 2 本の app へ推定を横展開する
3. Estimate JSON と portal 表示を section / overlap 詳細まで整える
4. 再推定比較の UI / API を整える
5. package metadata discovery を portal や比較導線に活かす
