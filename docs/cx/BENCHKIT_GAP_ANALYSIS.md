...
6. 使用量集計は node-hour 中心で、profile の allocation project ID と実行・使用量履歴を横断して見る導線はまだ未実装である。
...
| 拠点接続 | runner, scheduler, queue, site 条件を扱えること | `system.csv` と CI 生成が連携済み。`system_info.csv` を含めた責務分担、接続確認手順、障害切り分け、site onboarding checklist が docs に整理済みで、portal 側でも軽い configuration checks が見える。Portal execution profile と trigger runner により、manual / scheduled / repo-ref 契機は site-local DB から GitLab pipeline trigger へ接続済みである | docs 依存がまだ強く、site ごとの capability summary、接続前提、queue/resource 条件を自動確認する導線が未実装 | 使用量集計、申請、runner 分散運用に直結 | 高 |
