...
| 追加採取実行 | 通常実行と別に詳細推定入力の採取だけを共通入口から追加実行できること | `run_estimation_data_collection` 方向は整理済み | shell API、分岐条件、保存処理との接続は未実装 | 最優先 |
...
2. `run_estimation_data_collection` の共通入口を整える
