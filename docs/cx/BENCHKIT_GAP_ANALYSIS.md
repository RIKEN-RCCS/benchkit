...
- 推定結果自体の UUID / timestamp の保存
- portal での detailed metadata 表示
- UUID 比較 UI
- 複合推定の本実装
...
1. section ごとの package 指定と補助データ参照の実装口を整える
2. 推定結果自体の UUID / timestamp 保存を実装する
3. Estimate JSON と portal 表示を整える
4. `estimate.sh` のテンプレートまたは共通補助を整える
5. `qws` 以外に 1 から 2 本の app へ推定を横展開する
