...
`BK_GENESIS_GPU_MLP_PROFILE=true` の場合は GPU 推定入力の採取を優先し、`ncu` に `--kernel-name-base demangled --kernel-name regex:.*(inter_cell|intra_cell|build_pairlist).* --launch-skip 100 --launch-count 10` を既定で追加する。
これは `inter_cell` / `intra_cell` / `build_pairlist` 周辺の steady-state kernel を狙うための初期値であり、site や入力によって `BK_GENESIS_NCU_KERNEL_REGEX`, `BK_GENESIS_NCU_LAUNCH_SKIP`, `BK_GENESIS_NCU_LAUNCH_COUNT` で調整できる。
`BK_PROFILER_ARGS` を明示した場合は、その値を優先する。
