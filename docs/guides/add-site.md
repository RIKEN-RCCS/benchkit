...
6. `send_results` まで通ること
   - `RESULT_SERVER`、mTLS client certificate/key、Result Server 接続、`results/result*.json` の配置を確認します。
...
| send_results | `RESULT_SERVER` 未設定、mTLS client certificate/key、Result Server 接続、JSON 不足 | `scripts/result_server/send_results.sh`, Result Server log |
